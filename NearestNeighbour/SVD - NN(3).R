#SVD - NN(3).R
#This script fits a 3-parameter nearest-neighbour model, where 
#purine and pyrimidine sequence is considered, and directionality is not important,
...
NNlabels <- c("RR,YY","RY,YR","YR,RY")

#Function to count the 6 NN types in each oligo:
count_nn3 <- function(seq) {
  # Clean sequence
  s <- toupper(gsub("/", "", seq))
  ...
  # Define NN classes
  class_map <- list(
    RR_YY = c("CT", "AG", "GA", "TC", "AA", "TT", "GG", "CC"),
    RY_YR = c("GT", "AC", "AT", "GC"),
    YR_RY = c("TG", "CA", "TA", "CG")
  )
...
#Apply to data:
X <- t(sapply(Dataset$Sequence, count_nn3))
colnames(X) <- NNlabels
X <- as.matrix(X)

#Model is Y = XB; beta=(RR/YY, RY/YR, YR/RY)
...
rownames(cov_beta) <- colnames(cov_beta) <- c("RR/YY","RY/YR","YR/RY")
cov_beta
