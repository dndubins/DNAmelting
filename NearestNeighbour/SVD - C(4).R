#SVD - C(4).R
#This script fits a compositional model (4 nucleotides, C G A T) 
...
# Function to count the nucleotides in each oligo:
count_nt <- function(seq) {
  s <- toupper(gsub("/", "", seq))
  c(
...
#Make the counts happen:
X <- t(sapply(Dataset$Sequence, count_nt))
colnames(X) <- NNlabels
X <- as.matrix(X)

#Model is Y = XB; beta=(C,G,A,T)
