...
#the identity and order of the bases is important, 
...
NNlabels <-c(
  "AA,TT","AT", "TA", "CA,TG", "CT,AG", "CC,GG", "CG", "GA,TC", "GT,AC", "GC"
)
  
#Function to count the 6 NN types in each oligo:
count_nn10 <- function(seq) {
  # Clean sequence
  s <- toupper(gsub("/", "", seq))
...
#Apply to data:
X <- t(sapply(Dataset$Sequence, count_nn10))
colnames(X) <- NNlabels
X <- as.matrix(X)

#Model is Y = X*beta; beta=(10 NN parameters)
