...
#the identity of the bases is important but directionality is not considered plus end effects, 
...
NNlabels <-c(
  "TT,AA","TA,AT", "TG,GT,AC,CA", "AG,GA,TC,CT", "GG,CC", "GC,CG", "W"
)
  
count_nn7 <- function(seq) {
  # Clean sequence
  s <- toupper(gsub("/", "", seq))
  n <- nchar(s)
  
  # Extract nearest neighbors
  pairs <- substring(s, 1:(n - 1), 2:n)
  ...
  # ----- End effect W -----
  # +1 if 5' end is A or T
  # +1 if 3' end is A or T
  W <- as.integer(substr(s, 1, 1) %in% c("A", "T")) +
    as.integer(substr(s, n, n) %in% c("A", "T"))
...
#Apply to data:
X <- t(sapply(Dataset$Sequence, count_nn7))
colnames(X) <- NNlabels
X <- as.matrix(X)
